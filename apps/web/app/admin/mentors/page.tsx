...
import { fetchMentors, createMentor } from "@/lib/api/fellowship";
import { Mentor } from "@/types/fellowship";
import { UserRound, Plus, Building, Mail, Award, CheckCircle2 } from "lucide-react";
...
export default function MentorsPage() {
  ...
  const loadMentors = async () => {
    try {
      setLoading(true);
      const data = await fetchMentors({ search: search || undefined });
      setMentors(data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to provision mentor: " + (err as Error).message);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Provision, manage and assign industry leaders to fellowship teams.
          </p>
        </div>
      ...
      <div className="flex gap-4">
        <input
          type="text"
          placeholder="Search by mentor name, email, or company..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">Get started by provisioning your first mentor.</p>
        </div>
      ) : (
          ...
          {mentors.map((mentor) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-semibold text-slate-900">{mentor.full_name || "Mentor"}</h3>
                  <p className="text-xs text-slate-500">{mentor.designation || "Industry Expert"}</p>
                </div>
              ...
              <div className="space-y-1.5 text-xs text-slate-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>{mentor.company_name || "Company Unspecified"}</span>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>{mentor.years_of_experience || 0} years experience</span>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span>Active Teams: <strong className="text-slate-900">{mentor.assigned_teams_count}</strong></span>
                {mentor.linkedin_url && (
                  <a
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h2 className="text-lg font-bold text-slate-900">Provision New Mentor</h2>
            <form onSubmit={handleSubmit} className="space-y-3 text-sm">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700">Expertise (comma separated)</label>
                <input
                  type="text"
                  placeholder="e.g. AI/ML, Cloud Architecture, Product"

        ...
        <div className="w-full rounded-3xl border border-violet-100 bg-white p-8 shadow-xl shadow-violet-950/10">
          {complete ? (
            <div className="text-center">
              ...
              <Link href="/login?role=mentor" className="mt-7 inline-flex w-full items-center justify-center rounded-xl bg-violet-600 px-5 py-3 font-semibold text-white hover:bg-violet-700">Continue to sign in</Link>
            </div>
          ) : (
            <form onSubmit={submit}>
              <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-50 text-violet-600"><KeyRound className="h-6 w-6" /></div>
              ...
              <p className="mt-3 flex gap-2 text-xs leading-5 text-slate-500"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-violet-600" />Use 12+ characters, including upper- and lower-case letters, a number, and a symbol.</p>
              <button disabled={submitting} className="mt-6 w-full rounded-xl bg-violet-600 px-5 py-3 font-semibold text-white shadow-lg shadow-violet-600/20 hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60">{submitting ? "Activating account…" : "Activate mentor account"}</button>
            </form>
          )}
        </div>
      </section>
